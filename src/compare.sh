...
evaluate_mytool() {
    ...
    cp "$file" out_u.smt2
    local output
    output=$(timeout "$half_timeout" taskset -c 0-2 "$MYAPP_CMD" --verbose:1 --timeout:$((half_timeout - 1)) "$file" 2>&1)
    local exit_code=$?
    echo "$file" >> log.txt
    echo "$output" >> log.txt
    if [ $exit_code -eq 124 ]; then
...
cat "$BENCHMARK_FOLDER" | while read -r FILE; do
    ...
    echo "$FILE, $MY_TOOL_RESULT, $MYAPP_Z3_RESULT, $MYAPP_CVC5_RESULT, $MYAPP_BITW_RESULT" >> "$RESULTS_FILE"
done
